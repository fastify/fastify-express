...
test('Should support per path middlewares', (t, done) => {
  ...
  fastify.listen({ port: 0 }, (err, address) => {
    ...
    sget({
      ...
      t.assert.ifError(err)
      t.assert.ifError(res.headers['access-control-allow-origin'])
      fastify.close()
      done()
    })
  })
...
test('Should support complex middlewares', (t, done) => {
  t.plan(4)

  const fastify = Fastify()

  passport.use(new Strategy((token, cb) => {
    t.assert.strictEqual(token, '123456789')
    cb(null, { token })
  }))
...
test('Middleware chain (with errors) / 1', (t, done) => {
  ...
  function middleware2 (err, req, res, next) {
    t.assert.strictEqual(err.message, 'middleware1')
    t.assert.strictEqual(order.shift(), 2)
